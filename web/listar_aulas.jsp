<%-- 
    ...
    Created on : 16/12/2025, 15:24:00
    Author     : pmnch
--%>
...
<head>
    <meta charset="utf-8">
    <title>Aulas de Condução - Escola de Condução</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .badge-agendada { background: rgba(33,150,243,0.2); color: #2196F3; border: 1px solid #2196F3; }
        .badge-realizada { background: rgba(76,175,80,0.2); color: #4CAF50; border: 1px solid #4CAF50; }
        .badge-concluida { background: rgba(76,175,80,0.2); color: #4CAF50; border: 1px solid #4CAF50; }
        .badge-cancelada { background: rgba(244,67,54,0.2); color: #F44336; border: 1px solid #F44336; }
...
<body>
    ...
    <div class="container">
        ...
        <div class="content-card">
            <div class="page-header">
                ...
                <div class="btn-group">
                    <a href="adicionar_aula.jsp" class="btn btn-primary">
            ...
            <div class="search-bar">
                ...
                <input type="text" id="campoPesquisa" placeholder="Pesquisar por aluno ou instrutor..." onkeyup="filtrarTabela()">
            ...
            <div class="table-container">
                <%
                    ...
                    try {
                        ...
                        String sql = "SELECT ac.*, a.nome as nomeAluno, i.nome as nomeInstrutor, v.marca, v.modelo, v.matricula " +
                                     "FROM aula_conducao ac " +
                                     "LEFT JOIN aluno a ON ac.idAluno = a.id " +
                                     "LEFT JOIN instrutor i ON ac.idInstrutor = i.id " +
                                     "LEFT JOIN veiculo v ON ac.idVeiculo = v.id " +
                                     "ORDER BY ac.dataHoraInicio DESC";
                ...
                %>
                    <div class="empty-state">
                        ...
                        <p>Clica em "Agendar Aula" para começar</p>
                    </div>
                <%
                        } else {
                %>
                ...
                <%
                            while (rs.next()) {
                                ...
                                String badgeClass = "badge-agendada";
                                if ("Realizada".equals(estado) || "Concluída".equals(estado)) badgeClass = "badge-realizada";
                                else if ("Cancelada".equals(estado)) badgeClass = "badge-cancelada";
                                else if ("Agendada".equals(estado)) badgeClass = "badge-agendada";
                %>
                            <tr class="aula-row" data-aluno="<%= rs.getString("nomeAluno") != null ? rs.getString("nomeAluno").toLowerCase() : ""%>" data-inst="<%= rs.getString("nomeInstrutor") != null ? rs.getString("nomeInstrutor").toLowerCase() : ""%>">
                                <td><%= rs.getInt("id") %></td>
                                <td><strong><%= rs.getString("nomeAluno") %></strong></td>
                                <td><%= rs.getString("nomeInstrutor") %></td>
                                <td><%= rs.getString("marca") %> <%= rs.getString("modelo") %><br><small><%= rs.getString("matricula") %></small></td>
                                <td><%= rs.getTimestamp("dataHoraInicio") %></td>
                                <td><%= rs.getTimestamp("dataHoraFim") %></td>
                                <td><span class="badge <%= badgeClass %>"><%= estado %></span></td>
                                <td><%= rs.getString("observacoes") != null ? rs.getString("observacoes") : "-" %></td>
                                <td>
                                    <div class="actions">
                                        <a href="editar_aula.jsp?id=<%= rs.getInt("id") %>" class="btn btn-edit">
                                            <i class="fa fa-edit"></i> Editar
                                        </a>
                                        <a href="eliminar_aula.jsp?id=<%= rs.getInt("id") %>" class="btn btn-delete">
    ...
    function filtrarTabela() {
        ...
        var rows = document.querySelectorAll('.aula-row');
        var count = 0;
        rows.forEach(function(row) {
            var match = row.getAttribute('data-aluno').includes(input) || row.getAttribute('data-inst').includes(input);
            row.style.display = match ? '' : 'none';
            if (match) count++;
        });
        var total = rows.length;
        var contador = document.getElementById('contadorResultados');
        contador.textContent = input === '' ? total + ' aulas' : count + ' de ' + total + ' resultados';
        document.getElementById('semResultados').style.display = count === 0 && input !== '' ? 'block' : 'none';
    }
    window.onload = function() {
        var total = document.querySelectorAll('.aula-row').length;
        document.getElementById('contadorResultados').textContent = total + ' aulas';
    };
    </script>
    <script>
    function filtrarTabela() {
        ...
        var total = rows.length;
        document.getElementById('contadorResultados').textContent = input === '' ? total + ' aulas' : count + ' de ' + total + ' resultados';
        document.getElementById('semResultados').style.display = count === 0 && input !== '' ? 'block' : 'none';
    }
    window.onload = function() {
        document.getElementById('contadorResultados').textContent = document.querySelectorAll('.aula-row').length + ' aulas';
    };
    </script>
</body>
</html>
