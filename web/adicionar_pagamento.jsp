<%-- 
    Document   : adicionar_pagamento
    Created on : 06/01/2026, 09:56:14
    Author     : pmnch
--%>
...
<%
    ...
    if (request.getMethod().equals("POST")) {
        ...
        try {
            ...
            String sqlCheck = "SELECT COUNT(*) as total FROM pagamento WHERE idAluno = ?";
            pstmt = conn.prepareStatement(sqlCheck);
            pstmt.setInt(1, idAluno);
            rs = pstmt.executeQuery();
            boolean primeiroPagamento = false;
            if (rs.next()) primeiroPagamento = (rs.getInt("total") == 0);
            rs.close(); pstmt.close();

            String sql = "INSERT INTO pagamento (idAluno, valor, metodo, referenciaPagamento, dataPagamento, descricao) VALUES (?, ?, ?, ?, ?, ?)";
            ...
            if (resultado > 0) {
                if (primeiroPagamento) {
                    String sqlAtivar = "UPDATE t_utilizador SET ativo = 1 WHERE idAluno = ?";
                    pstmt = conn.prepareStatement(sqlAtivar);
                    pstmt.setInt(1, idAluno);
                    pstmt.executeUpdate(); pstmt.close();
                }
                response.sendRedirect("listar_pagamentos.jsp?msg=success"); return;
            }
        } catch (Exception e) {
            e.printStackTrace(); mensagemErro = "Erro ao registar pagamento: " + e.getMessage();
        } finally {
...
<head>
    ...
    <title>Registar Pagamento - Escola de Condução</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .info-box { background: rgba(255,193,7,0.08); border: 1px solid rgba(255,193,7,0.25); border-radius: 12px; padding: 18px 22px; margin-bottom: 28px; color: rgba(255,255,255,0.8); }
        .info-box h3 { color: #FFC107; margin-bottom: 10px; font-size: 1rem; display: flex; align-items: center; gap: 8px; }
    ...
    <div class="container">
        <div class="form-card">
            <div class="form-header">
                ...
                <h1>Registar Novo Pagamento</h1>
                <p>Preencha os dados abaixo para registar um pagamento</p>
            </div>

            <% if ("success".equals(request.getParameter("msg"))) { %>
                <div class="alert alert-success"><i class="fa fa-check-circle fa-2x"></i><div><strong>Sucesso!</strong><br>Pagamento registado com sucesso!</div></div>
            <% } %>
            <% if (mensagemErro != null) { %>
                <div class="alert alert-error"><i class="fa fa-exclamation-triangle fa-2x"></i><div><strong>Erro!</strong><br><%= mensagemErro %></div></div>
            <% } %>

            <div class="info-box">
                <h3><i class="fa fa-info-circle"></i> Informação sobre Pagamentos</h3>
                <p>
                    <strong>Pronto Pagamento:</strong> €550,00 (valor único)<br>
                    <strong>Parcelado:</strong> €600,00 (6 prestações de €100,00)<br><br>
                    ⚠️ <strong>Importante:</strong> O <strong>primeiro pagamento</strong> de um aluno ativa a sua conta automaticamente!
                </p>
            </div>

            <form method="POST" action="adicionar_pagamento.jsp">
                <div class="form-grid">
                    <div class="form-group">
                        <label><i class="fa fa-user"></i> Aluno <span class="required">*</span></label>
                        <select name="idAluno" required>
                            ...
                            <%
                                ...
                                try {
                                    conn = ConexaoBD.getConnection();
                                    String sql = "SELECT a.id, a.nome, a.tipoPagamento, a.valorTotal, (SELECT COUNT(*) FROM pagamento p WHERE p.idAluno = a.id) as totalPag FROM aluno a ORDER BY a.nome";
                                    pstmt = conn.prepareStatement(sql);
                                    rs = pstmt.executeQuery();
                                    while (rs.next()) {
                                        boolean primeiro = (rs.getInt("totalPag") == 0);
                            %>
                                <option value="<%= rs.getInt("id") %>">
                                    <%= rs.getString("nome") %> (<%= rs.getString("tipoPagamento") %> - €<%= String.format("%.2f", rs.getDouble("valorTotal")) %>)<%= primeiro ? " [PENDENTE]" : "" %>
                                </option>
                            <%
                                    }
                                } catch (Exception e) { e.printStackTrace(); }
                                finally { try { if(rs!=null)rs.close(); if(pstmt!=null)pstmt.close(); if(conn!=null)conn.close(); } catch(SQLException e){} }
                            %>
                        </select>
                    </div>
                    <div class="form-group">
                        <label><i class="fa fa-euro-sign"></i> Valor <span class="required">*</span></label>
                        <input type="number" step="0.01" name="valor" placeholder="Ex: 100.00" required>
                    </div>
                    <div class="form-group">
                        <label><i class="fa fa-credit-card"></i> Método de Pagamento <span class="required">*</span></label>
                    ...
                    <div class="form-group">
                        <label><i class="fa fa-hashtag"></i> Referência de Pagamento</label>
                        <input type="text" name="referenciaPagamento" placeholder="Ex: REF123456">
                    </div>
                    <div class="form-group">
                        <label><i class="fa fa-calendar"></i> Data do Pagamento <span class="required">*</span></label>
                        <input type="date" name="dataPagamento" required>
                    </div>
                    <div class="form-group full-width">
                        <label><i class="fa fa-comment"></i> Descrição <span class="required">*</span></label>
                        <textarea name="descricao" required placeholder="Ex: Pagamento inicial - inscrição"></textarea>
                    </div>
                </div>
                <div class="btn-group">
                    <a href="listar_pagamentos.jsp" class="btn btn-secondary"><i class="fa fa-times"></i> Cancelar</a>
                    <button type="submit" class="btn btn-primary"><i class="fa fa-save"></i> Registar Pagamento</button>
                </div>
            </form>
        </div>
    </div>
