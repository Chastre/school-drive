<%-- 
    Document   : editar_instrutor
    Created on : 16/12/2025, 14:50:53
    Author     : pmnch
--%>
...
<%
    ...
    if (request.getMethod().equals("POST")) {
        ...
        String especializado = request.getParameter("especializado");
        ...
        try {
            ...
            String sql = "UPDATE instrutor SET nome=?, email=?, telemovel=?, morada=?, dataNascimento=?, dataContratacao=?, especializado=?, ativo=? WHERE id=?";
            ...
            pstmt.setDate(6, Date.valueOf(dataContratacao));
            pstmt.setString(7, especializado);
            pstmt.setBoolean(8, "1".equals(ativo));
            pstmt.setInt(9, id);
            
            int resultado = pstmt.executeUpdate();
            
            if (resultado > 0) {
                response.sendRedirect("listar_instrutores.jsp?msg=success");
                return;
            }
    ...
    if (idParam == null || idParam.isEmpty()) {
        response.sendRedirect("listar_instrutores.jsp");
        return;
    }
    ...
    try {
        ...
        if (rs.next()) {
            ...
            dataContratacao = rs.getDate("dataContratacao").toString();
            especializado = rs.getString("especializado");
...
<head>
    ...
    <title>Editar Instrutor - Escola de Condução</title>
    ...
    <div class="container">
        <div class="form-card">
            <div class="form-header">
                <div class="icon-wrap"><i class="fa fa-chalkboard-teacher"></i></div>
                <h1>Editar Instrutor</h1>
                <p>Atualize os dados do instrutor</p>
            </div>
            ...
            <form method="POST" action="editar_instrutor.jsp">
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    ...
                    <input type="tel" id="telemovel" name="telemovel" value="<%= telemovel %>" required>
                </div>
                ...
                <div class="form-group">
                    <label for="dataContratacao">Data de Contratação <span class="required">*</span></label>
                    <input type="date" id="dataContratacao" name="dataContratacao" value="<%= dataContratacao %>" required>
                </div>

                <div class="form-group">
                    <label for="especializado">Especialização <span class="required">*</span></label>
                    <select id="especializado" name="especializado" required>
                        <option value="">Selecione...</option>
                        <option value="A" <%= especializado.equals("A") ? "selected" : "" %>>A - Motociclos</option>
                        <option value="B" <%= especializado.equals("B") ? "selected" : "" %>>B - Ligeiros</option>
                        <option value="C" <%= especializado.equals("C") ? "selected" : "" %>>C - Pesados</option>
                        <option value="D" <%= especializado.equals("D") ? "selected" : "" %>>D - Autocarros</option>
                        <option value="A,B" <%= especializado.equals("A,B") ? "selected" : "" %>>A e B</option>
                        <option value="B,C" <%= especializado.equals("B,C") ? "selected" : "" %>>B e C</option>
                        <option value="A,B,C" <%= especializado.equals("A,B,C") ? "selected" : "" %>>A, B e C</option>
                        <option value="A,B,C,D" <%= especializado.equals("A,B,C,D") ? "selected" : "" %>>Todas</option>
                    </select>
                </div>

                <div class="form-group" style="grid-column: 1 / -1;">
                    <label for="morada">Morada <span class="required">*</span></label>
                    <textarea id="morada" name="morada" required><%= morada %></textarea>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="checkbox-group">
                        <div class="checkbox-item">
                            <input type="radio" id="ativo1" name="ativo" value="1" <%= ativo ? "checked" : "" %>>
                            <label for="ativo1" style="margin: 0;">✓ Ativo</label>
                        </div>
                        <div class="checkbox-item">
                            <input type="radio" id="ativo0" name="ativo" value="0" <%= !ativo ? "checked" : "" %>>
                            <label for="ativo0" style="margin: 0;">✗ Inativo</label>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="btn-group">
                <a href="listar_instrutores.jsp" class="btn btn-secondary">❌ Cancelar</a>
                <button type="submit" class="btn btn-primary">💾 Atualizar Instrutor</button>
            </div>
        </form>
        </div>
    </div>
