<%-- 
    Document   : processar_agendamento
    Created on : 19/01/2026, 12:09:18
    Author     : pmnch
--%>
...
<%
    ...
    try {
        ...
        // ===== BUSCAR VEÍCULO =====
        // Primeiro tenta pegar veículo do instrutor
        Integer idVeiculo = null;
        
        pstmt = conn.prepareStatement("SELECT idVeiculo FROM instrutor WHERE id = ?");
        pstmt.setInt(1, idInstrutor);
        rs = pstmt.executeQuery();
        if (rs.next()) {
        ...
        // Se não tem, pega qualquer veículo ativo
        if (idVeiculo == null || idVeiculo == 0) {
            pstmt = conn.prepareStatement("SELECT id FROM veiculo WHERE ativo = 1 ORDER BY id LIMIT 1");
            rs = pstmt.executeQuery();
            if (rs.next()) {
        ...
        // ÚLTIMA TENTATIVA: Usa ID 1 fixo
        if (idVeiculo == null || idVeiculo == 0) {
            idVeiculo = 1;
        }

        // ===== INSERIR AULA COM VEÍCULO =====
        pstmt = conn.prepareStatement(
        ...
        e.printStackTrace();
        response.sendRedirect("agendar_aula.jsp?erro=exception&msg=" + java.net.URLEncoder.encode(e.getMessage(), "UTF-8"));
    } finally {
        try {
            if (rs != null) rs.close();
