<%-- 
    Document   : eliminar_manutencao
    Created on : 18/12/2025, 00:20:44
    Author     : pmnch
--%>
...
<%
    ...
    if (idParam == null || idParam.isEmpty()) {
        response.sendRedirect("listar_manutencoes.jsp?msg=error"); return;
    }
    ...
    if ("sim".equals(request.getParameter("confirmar"))) {
        ...
        try {
            ...
            pstmt = conn.prepareStatement("DELETE FROM manutencao_veiculo WHERE id = ?");
            pstmt.setInt(1, id);
            int resultado = pstmt.executeUpdate();
            if (resultado > 0) { response.sendRedirect("listar_manutencoes.jsp?msg=success"); }
            else { response.sendRedirect("listar_manutencoes.jsp?msg=error"); }
        } catch (Exception e) {
            e.printStackTrace();
            response.sendRedirect("listar_manutencoes.jsp?msg=error");
        } finally {
...
<head>
    ...
    <title>Eliminar Manutenção - Escola de Condução</title>
...
<body>
    ...
    <div class="overlay">
        <div class="modal">
            ...
            <p>Tens a certeza que queres eliminar este registo de manutenção?</p>
            ...
            <div class="btn-group">
                <a href="listar_manutencoes.jsp" class="btn btn-secondary">
                    <i class="fa fa-times"></i> Cancelar
                </a>
                <a href="eliminar_manutencao.jsp?id=<%= id %>&confirmar=sim" class="btn btn-danger">
