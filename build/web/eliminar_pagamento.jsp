<%-- 
    Document   : eliminar_pagamento
    Created on : 18/12/2025, 00:07:59
    Author     : pmnch
--%>
...
<%
    ...
    if (idParam == null || idParam.isEmpty()) {
        response.sendRedirect("listar_pagamentos.jsp?msg=error"); return;
    }
    ...
    if ("sim".equals(request.getParameter("confirmar"))) {
        ...
        try {
            ...
            pstmt = conn.prepareStatement("DELETE FROM pagamento WHERE id = ?");
            pstmt.setInt(1, id);
            int resultado = pstmt.executeUpdate();
            if (resultado > 0) { response.sendRedirect("listar_pagamentos.jsp?msg=success"); }
            else { response.sendRedirect("listar_pagamentos.jsp?msg=error"); }
        } catch (Exception e) {
            e.printStackTrace();
            response.sendRedirect("listar_pagamentos.jsp?msg=error");
        } finally {
...
<head>
    ...
    <title>Eliminar Pagamento - Escola de Condução</title>
...
<body>
    ...
    <div class="overlay">
        <div class="modal">
            ...
            <h2>Eliminar Pagamento</h2>
            <p>Tens a certeza que queres eliminar este pagamento?</p>
            ...
            <div class="btn-group">
                <a href="listar_pagamentos.jsp" class="btn btn-secondary">
                    <i class="fa fa-times"></i> Cancelar
                </a>
                <a href="eliminar_pagamento.jsp?id=<%= id %>&confirmar=sim" class="btn btn-danger">
