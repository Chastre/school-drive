<%-- 
    Document   : listar_veiculos
    Created on : 16/12/2025, 14:56:25
    Author     : pmnch
--%>
...
<head>
    <meta charset="utf-8">
    <title>Gestão de Veículos - Escola de Condução</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .badge-excelente { background: rgba(76,175,80,0.2); color: #4CAF50; border: 1px solid #4CAF50; }
        .badge-bom { background: rgba(33,150,243,0.2); color: #2196F3; border: 1px solid #2196F3; }
        .badge-razoavel { background: rgba(255,193,7,0.2); color: #FFC107; border: 1px solid #FFC107; }
        .badge-mau { background: rgba(244,67,54,0.2); color: #F44336; border: 1px solid #F44336; }
...
<body>
    ...
    <div class="container">
        ...
        <div class="content-card">
            <div class="page-header">
                ...
                <div class="btn-group">
                    <a href="adicionar_veiculo.jsp" class="btn btn-primary">
            ...
            <div class="search-bar">
                ...
                <input type="text" id="campoPesquisa" placeholder="Pesquisar por matrícula, marca ou modelo..." onkeyup="filtrarTabela()">
            ...
            <div class="table-container">
                <%
                    ...
                    try {
                        ...
                        rs = stmt.executeQuery("SELECT * FROM veiculo ORDER BY id DESC");
                        if (!rs.isBeforeFirst()) {
                %>
                    <div class="empty-state">
                        <i class="fa fa-car"></i>
                        <h3>Nenhum veículo registado</h3>
                        <p>Clica em "Adicionar Veículo" para começar</p>
                    </div>
                <%
                        } else {
                %>
                ...
                <%
                            while (rs.next()) {
                                ...
                                String badgeClass = "badge-mau";
                                if ("Excelente".equals(estado)) badgeClass = "badge-excelente";
                                else if ("Bom".equals(estado)) badgeClass = "badge-bom";
                                else if ("Razoavel".equals(estado) || "Razoável".equals(estado)) badgeClass = "badge-razoavel";
                %>
                            <tr class="vei-row" data-mat="<%= rs.getString("matricula") != null ? rs.getString("matricula").toLowerCase() : ""%>" data-marca="<%= rs.getString("marca") != null ? rs.getString("marca").toLowerCase() : ""%>" data-modelo="<%= rs.getString("modelo") != null ? rs.getString("modelo").toLowerCase() : ""%>">
                                <td><%= rs.getInt("id") %></td>
                                ...
                                <td><%= String.format("%,d km", rs.getInt("quilometragemAtual")) %></td>
                                <td>
                                    <div class="actions">
                                        <a href="editar_veiculo.jsp?id=<%= rs.getInt("id") %>" class="btn btn-edit">
                                            <i class="fa fa-edit"></i> Editar
                                        </a>
                                        <a href="eliminar_veiculo.jsp?id=<%= rs.getInt("id") %>" class="btn btn-delete">
    ...
    function filtrarTabela() {
        ...
        var rows = document.querySelectorAll('.vei-row');
        var count = 0;
        rows.forEach(function(row) {
            var match = row.getAttribute('data-mat').includes(input) || row.getAttribute('data-marca').includes(input) || row.getAttribute('data-modelo').includes(input);
            row.style.display = match ? '' : 'none';
            if (match) count++;
        });
        var total = rows.length;
        var contador = document.getElementById('contadorResultados');
        contador.textContent = input === '' ? total + ' veículos' : count + ' de ' + total + ' resultados';
        document.getElementById('semResultados').style.display = count === 0 && input !== '' ? 'block' : 'none';
    }
    window.onload = function() {
        var total = document.querySelectorAll('.vei-row').length;
        document.getElementById('contadorResultados').textContent = total + ' veículos';
    };
    </script>
    <script>
    function filtrarTabela() {
        ...
        var total = rows.length;
        document.getElementById('contadorResultados').textContent = input === '' ? total + ' veículos' : count + ' de ' + total + ' resultados';
        document.getElementById('semResultados').style.display = count === 0 && input !== '' ? 'block' : 'none';
    }
    window.onload = function() {
        document.getElementById('contadorResultados').textContent = document.querySelectorAll('.vei-row').length + ' veículos';
    };
    </script>
</body>
</html>
