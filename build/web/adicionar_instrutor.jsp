...
<%
    ...
    if (request.getMethod().equals("POST")) {
        ...
        try {
            ...
            String sql = "INSERT INTO instrutor (nome, email, telemovel, morada, dataNascimento, dataContratacao, ativo) VALUES (?, ?, ?, ?, ?, ?, TRUE)";
            ...
            pstmt.setString(6, dataContratacao);
            int resultado = pstmt.executeUpdate();
            if (resultado > 0) { response.sendRedirect("listar_instrutores.jsp?msg=success"); return; }
            else { response.sendRedirect("adicionar_instrutor.jsp?msg=error"); return; }
        } catch (Exception e) {
            e.printStackTrace();
            response.sendRedirect("adicionar_instrutor.jsp?msg=error"); return;
        } finally {
...
<head>
    ...
    <title>Adicionar Instrutor - Escola de Condução</title>
    ...
    <div class="container">
        <div class="form-card">
            <div class="form-header">
                ...
                <h1>Adicionar Novo Instrutor</h1>
                <p>Preencha os dados abaixo para registar um novo instrutor</p>
            </div>

            <% if ("success".equals(request.getParameter("msg"))) { %>
                <div class="alert alert-success"><i class="fa fa-check-circle fa-2x"></i><div><strong>Sucesso!</strong><br>Instrutor adicionado com sucesso!</div></div>
            <% } %>
            <% if ("error".equals(request.getParameter("msg"))) { %>
                <div class="alert alert-error"><i class="fa fa-exclamation-triangle fa-2x"></i><div><strong>Erro!</strong><br>Erro ao adicionar instrutor! Verifica os dados.</div></div>
            <% } %>

            <form method="POST" action="adicionar_instrutor.jsp">
                <div class="form-grid">
                    <div class="form-group full-width">
                        <label><i class="fa fa-user"></i> Nome Completo <span class="required">*</span></label>
                        <input type="text" name="nome" required placeholder="Ex: António Silva">
                    </div>
                    <div class="form-group">
                        <label><i class="fa fa-envelope"></i> Email <span class="required">*</span></label>
                        <input type="email" name="email" required placeholder="antonio@exemplo.com">
                    </div>
                    <div class="form-group">
                        <label><i class="fa fa-phone"></i> Telemóvel <span class="required">*</span></label>
                        <input type="tel" name="telemovel" required placeholder="912345678">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label><i class="fa fa-calendar"></i> Data de Contratação <span class="required">*</span></label>
                        <input type="date" name="dataContratacao" required>
                    </div>
                    <div class="form-group full-width">
                        <label><i class="fa fa-home"></i> Morada <span class="required">*</span></label>
                        <textarea name="morada" required placeholder="Rua, Número, Código Postal, Cidade"></textarea>
                    </div>
                </div>
                <div class="btn-group">
                    <a href="listar_instrutores.jsp" class="btn btn-secondary"><i class="fa fa-times"></i> Cancelar</a>
                    <button type="submit" class="btn btn-primary"><i class="fa fa-save"></i> Guardar Instrutor</button>
                </div>
            </form>
        </div>
    </div>
